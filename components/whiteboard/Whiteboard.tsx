...
export function Whiteboard({
  ...
  const suppressUndoRef = useRef(false); // ✅ KEY FIX
  ...
  const pushUndo = useCallback(() => {
    if (suppressUndoRef.current) return; // ✅ Skip during shape drawing
    const canvas = fabricRef.current;
    if (!canvas) return;
  ...
  useEffect(() => {
    ...
    if (activeTool === "pen") {
      ...
      const handleMouseDown = (opt: any) => {
        // ✅ Suppress ALL object events during drawing
      ...
      const handleMouseMove = (opt: any) => {
        ...
        if (activeTool === "rectangle" || activeTool === "triangle") {
          shape.set({
            left: Math.min(pointer.x, sp.x),
            top: Math.min(pointer.y, sp.y),
            width: Math.abs(pointer.x - sp.x) || 1,
            height: Math.abs(pointer.y - sp.y) || 1,
          });
          shape.setCoords();
        } else if (activeTool === "circle") {
          shape.set({
            left: Math.min(pointer.x, sp.x),
            top: Math.min(pointer.y, sp.y),
            rx: Math.abs(pointer.x - sp.x) / 2 || 1,
            ry: Math.abs(pointer.y - sp.y) / 2 || 1,
          });
          shape.setCoords();
        } else if (activeTool === "line" || activeTool === "arrow") {
        ...
        // ✅ requestRenderAll is smoother than renderAll during drag
      ...
      const handleMouseUp = () => {
        ...
        const shape = drawingShapeRef.current;
        if (shape) {
          // ✅ Re-enable interaction on the final shape
          shape.set({
            selectable: true,
            evented: true,
            hasControls: true,
          ...
          // ✅ Re-enable undo AFTER shape is fully drawn
  ...
  const addText = async (sticky = false) => {
    ...
    if (sticky) {
      const rect = new fabric.Rect({
        left: 80, top: 80, width: 160, height: 120,
        fill: "#fef08a", stroke: "#ca8a04", strokeWidth: 1,
        rx: 4, ry: 4,
      });
      const text = new fabric.IText("Sticky note", {
        left: 160, top: 140,
        fill: "#713f12", fontSize: 14,
        fontFamily: "sans-serif",
        originX: "center", originY: "center",
      });
      const group = new fabric.Group([rect, text], { left: 80, top: 80 });
  ...
  const handleClear = () => {
    ...
    suppressUndoRef.current = true;
    canvas.clear();
    canvas.setBackgroundColor(boardColor, () => {
      canvas.renderAll();
      suppressUndoRef.current = false;
      pushUndo();
    });
  ...
  const COLORS = [
    ...
    "#1e40af","#065f46","#7c2d12","#1e1b4b","#134e4a",
  ];

  const BOARD_COLORS = [
    "#ffffff","#f8fafc","#1a1a2e","#0f0f13","#1e293b",
    "#fef9c3","#ecfdf5","#eff6ff","#18181b","#0f172a",
  ];

  const FONTS = ["sans-serif","serif","monospace","Georgia","Verdana","Arial","Trebuchet MS","Impact"];

  const ToolBtn = ({
    tool, icon, title, shortcut, onClick
  }: {
    ...
    <button
      ...
      className={`relative w-7 h-7 flex items-center justify-center rounded-md transition-all text-xs font-medium
    ...
    <div className="flex flex-col w-full h-full overflow-hidden bg-white" style={{ fontFamily: "system-ui, sans-serif" }}>
      ...
      <div className="flex items-center gap-0.5 px-2 py-1 bg-gray-50 border-b border-gray-200 shrink-0 flex-wrap">
        ...
        <div className="flex items-center gap-0.5 pr-2 border-r border-gray-200">
          ...
          />
          <ToolBtn title="Export SVG" onClick={handleExportSVG}
            icon={<span className="text-[8px] font-bold">SVG</span>}
          />
        </div>
        ...
        <div className="flex items-center gap-1 px-2 border-r border-gray-200">
          ...
          <button onClick={() => { setZoom(100); fabricRef.current?.setZoom(1); fabricRef.current?.renderAll(); }}
            className="text-[9px] text-gray-400 hover:text-gray-600 px-1">Reset</button>
        </div>

        <div className="ml-auto flex items-center gap-2">
          <span className={`w-1.5 h-1.5 rounded-full ${isReady ? "bg-emerald-400" : "bg-amber-400"}`} />
          {savedAt && <span className="text-[9px] text-gray-400">Saved {savedAt}</span>}
        </div>
      </div>
      ...
      <div className="flex items-center gap-0.5 px-2 py-1 bg-white border-b border-gray-200 shrink-0 flex-wrap">
        ...
        <div className="flex items-center gap-0.5 pr-2 border-r border-gray-200">
          ...
          <ToolBtn tool="spray" title="Spray"
            icon={<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 3l3 9"/><circle cx="7.5" cy="14.5" r=".5" fill="currentColor"/><circle cx="11.5" cy="12.5" r=".5" fill="currentColor"/><circle cx="9.5" cy="10.5" r=".5" fill="currentColor"/><circle cx="13.5" cy="10.5" r=".5" fill="currentColor"/><circle cx="15.5" cy="14.5" r=".5" fill="currentColor"/><path d="m6 3 3 3"/><path d="M3 6h3"/></svg>} />
        ...
        <div className="flex items-center gap-0.5 px-2 border-r border-gray-200">
          ...
          <ToolBtn tool="arrow" title="Arrow"
            icon={<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="3" y1="21" x2="21" y2="3"/><polyline points="11 3 21 3 21 13"/></svg>} />
        </div>
        ...
        <div className="flex items-center gap-1.5 px-2 border-r border-gray-200 relative" ref={colorPickerRef}>
          <div className="flex flex-col items-center gap-0.5">
            ...
            <button
              ...
              className="w-6 h-6 rounded border-2 border-gray-300 shadow-sm hover:border-gray-400 transition"
          ...
          <div className="flex flex-col items-center gap-0.5">
            <span className="text-[8px] text-gray-400">Board</span>
            <button
              onClick={() => setShowColorPicker(showColorPicker === "board" ? null : "board")}
              className="w-6 h-6 rounded border-2 border-dashed border-gray-400 hover:border-gray-600 transition"
          ...
          {showColorPicker && (
            <div className="absolute top-full left-0 mt-1 z-50 bg-white border border-gray-200 rounded-xl shadow-xl p-3 w-52">
              ...
              <div className="grid grid-cols-5 gap-1.5 mb-2">
                {(showColorPicker === "board" ? BOARD_COLORS : COLORS).map((c) => (
                  <button
                    key={c}
                    onClick={() => {
                      if (showColorPicker === "stroke") { setActiveColor(c); if (activeTool === "eraser") setActiveTool("pen"); }
                    ...
                    style={{
                      backgroundColor: c,
                      borderColor: (showColorPicker === "stroke" ? activeColor : showColorPicker === "fill" ? fillColor : boardColor) === c ? "#3b82f6" : "#e5e7eb",
              ...
              <div className="flex items-center gap-2 mt-1">
                <input
                  type="color"
                  value={
                    showColorPicker === "stroke" ? activeColor
                    : showColorPicker === "fill" ? (fillColor === "transparent" ? "#ffffff" : fillColor)
                    : boardColor
                  }
                  onChange={(e) => {
                ...
                {showColorPicker === "fill" && (
                  <button
                    ...
                    className="ml-auto text-[9px] text-red-400 hover:text-red-600"
                  >
                    None
                  </button>
                )}
      ...
      <div className="flex items-center gap-1 px-2 py-1 bg-gray-50 border-b border-gray-200 shrink-0 flex-wrap">
        ...
        <button onClick={() => { setIsBold(!isBold); updateSelectedText({ fontWeight: !isBold ? "bold" : "normal" }); }}
          className={`w-6 h-6 flex items-center justify-center rounded text-xs font-bold transition ${isBold ? "bg-blue-100 text-blue-700" : "text-gray-500 hover:bg-gray-100"}`}>B</button>
        <button onClick={() => { setIsItalic(!isItalic); updateSelectedText({ fontStyle: !isItalic ? "italic" : "normal" }); }}
          className={`w-6 h-6 flex items-center justify-center rounded text-xs italic font-medium transition ${isItalic ? "bg-blue-100 text-blue-700" : "text-gray-500 hover:bg-gray-100"}`}>I</button>
        <button onClick={() => { setIsUnderline(!isUnderline); updateSelectedText({ underline: !isUnderline }); }}
          className={`w-6 h-6 flex items-center justify-center rounded text-xs underline font-medium transition ${isUnderline ? "bg-blue-100 text-blue-700" : "text-gray-500 hover:bg-gray-100"}`}>U</button>
        <div className="w-px h-4 bg-gray-200 mx-0.5" />
        {(["left","center","right"] as const).map((align) => (
          <button key={align} onClick={() => updateSelectedText({ textAlign: align })}
            className="w-6 h-6 flex items-center justify-center rounded text-gray-500 hover:bg-gray-100 transition" title={`Align ${align}`}>
